...
{
	std::vector<Out_T> ret_val(std::size(in) - 1);
	auto it_out = std::begin(ret_val);
	try
	{
		for (auto it = std::next(std::begin(in), 1); it != std::end(in);)
		{
			*it_out++ = funct(*it++);
		}
	}
...
public:
	...
	{
		return std::vector<std::string>(std::next(std::begin(m_return_val)), std::end(m_return_val));
	}
